      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        ...
        <section className="mt-8">
          {!goal || goalEditing ? (
            ...
            goalSummary && (
              <div className="flex flex-col gap-3 rounded-xl border border-zinc-800 bg-zinc-900/80 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  {goalSummary.onTrack != null && (
                    <span className="ml-2">
                      ...
                      {goalSummary.onTrack ? (
                        ...
                        <span className="text-[#ef4444]">Behind ✗</span>
                      )}
                    </span>
                  )}
                </p>
                <div className="flex shrink-0 flex-wrap gap-2">
                  <button
                    ...
                    className="rounded-lg border border-[#f97316]/70 bg-[#f97316]/10 px-3 py-1.5 text-sm font-medium text-[#fb923c] hover:bg-[#f97316]/20"
                  >
                    Edit Goal
                  </button>
                  <button
        ...
        {goalDeleteModalOpen && (
          <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4">
              ...
              >
                <h2 id="delete-goal-dialog-title" className="text-lg font-bold text-white">
                  Delete weight goal?
                </h2>
        ...
        <section className="mt-10">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">Daily log</h2>
          <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-900/50 p-6">
            {todayEntry && editRowId !== todayEntry.id ? (
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-sm text-zinc-400">Today ({formatShortDate(todayIso)})</p>
                  <p className="mt-1 text-4xl font-bold tabular-nums text-white">
                    {kgToLbs(Number(todayEntry.weight_kg))?.toFixed(1)} <span className="text-xl text-zinc-500">lbs</span>
                  </p>
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    className="rounded-lg border border-zinc-600 px-4 py-2 text-sm font-medium text-white"
                  >
                    Edit
                  </button>
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-400">Log today&apos;s weight (lbs)</label>
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center">
                  <input
                    ...
                    placeholder="0.0"
                    className="h-16 min-w-0 flex-1 rounded-xl border-2 border-[#f97316]/60 bg-zinc-950 px-5 text-3xl font-bold tabular-nums text-white outline-none focus:border-[#f97316]"
                  />
                  <button
                    type="button"
                    onClick={() =>
                    ...
                    className="h-14 shrink-0 rounded-xl bg-[#f97316] px-8 text-base font-semibold text-[#0a0a0a] disabled:opacity-50"
                  >
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              ...
              <tbody>
                {last30Rows.map((iso, idx) => {
                  ...
                  const stripe = idx % 2 === 0 ? "bg-zinc-900/80" : "bg-zinc-950/80";
                  return (
                    <tr key={iso} className={`border-b border-zinc-800/80 ${stripe}`}>
                      <td className="px-3 py-2 text-zinc-300">{formatShortDate(iso)}</td>
                      <td className="px-3 py-2 text-zinc-500">{dayOfWeekLabel(iso)}</td>
                        ...
                        {row ? (
                          editRowId === row.id ? (
                            <input
                              ...
                              className="w-24 rounded border border-zinc-600 bg-zinc-950 px-2 py-1"
                            />
                          ) : (
        ...
        {/* Weekly summary */}
        <section className="mt-10">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">Weekly summary</h2>
          <p className="mt-1 text-sm text-zinc-500">Weeks run Monday–Sunday. Target is the linear plan to your goal date.</p>
                      ...
                      row.onTrack === true
                        ...
                        : row.onTrack === false
                          ...
                          : i % 2 === 0
                            ...
                            : "bg-zinc-950/80";
                    return (
                      <tr key={row.weekMon} className={`border-b border-zinc-800/80 ${hi}`}>
                        <td className="px-2 py-2 text-zinc-300">{formatShortDate(row.weekMon)}</td>
                        <td className="px-2 py-2 tabular-nums text-zinc-400">{row.targetLbs.toFixed(1)}</td>
                        ...
                        {row.days.map((v, j) => (
                          <td key={j} className="px-1 py-2 text-center tabular-nums text-zinc-300">
                            {v != null ? v.toFixed(1) : "—"}
                          </td>
                        ))}
        ...
        {goal && trajectory && (
          <section className="mt-10">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">Running projection</h2>
            {scheduleMessage && (
              <p
                ...
                <tbody>
                  {projectionRows.length === 0 ? (
                    ...
                    projectionRows.map((r, idx) => (
                      <tr
                        key={r.weekMon}
                        className={`border-b border-zinc-800/80 ${idx % 2 === 0 ? "bg-zinc-900/80" : "bg-zinc-950/80"}`}
                      >
                        <td className="px-3 py-2 text-zinc-300">{formatShortDate(r.weekMon)}</td>
                        <td className="px-3 py-2 tabular-nums text-[#f97316]">{r.projectedLbs.toFixed(1)}</td>
                        <td className="px-3 py-2 tabular-nums text-zinc-400">{r.targetLbs.toFixed(1)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
        ...
        <section className="mt-10 space-y-6">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">Charts</h2>
